"""Squashed baseline for dev database.

Revision ID: 001_initial_squashed
Revises:
...
SCHEMA = "public"
ROLE_ENUM_VALUES = ("inbound", "assistant")
APPT_STATUS_ENUM_VALUES = ("pending", "confirmed", "cancelled")
...
def _drop_enum(name: str) -> None:
    ...
    op.execute(sa.text(f'DROP TYPE IF EXISTS "{SCHEMA}"."{name}"'))


def _drop_extension(extension: str) -> None:
    bind = op.get_bind()
    if bind.dialect.name != "postgresql":
        return
...
def upgrade() -> None:
    ...
    _ensure_enum("role_enum", ROLE_ENUM_VALUES)
    _ensure_enum("appt_status_enum", APPT_STATUS_ENUM_VALUES)

    role_enum_kwargs: dict[str, object] = {"name": "role_enum"}
    appt_status_kwargs: dict[str, object] = {"name": "appt_status_enum"}
    ...
    role_enum = sa.Enum(*ROLE_ENUM_VALUES, **role_enum_kwargs)
    appt_status_enum = sa.Enum(*APPT_STATUS_ENUM_VALUES, **appt_status_kwargs)

    op.create_table(
        "tenants",
        sa.Column("id", sa.String(length=255), nullable=False),
        sa.Column("phone_id", sa.String(length=255), nullable=False, unique=True),
    ...
    op.create_index(op.f("ix_tenants_id"), "tenants", ["id"], schema=SCHEMA)
    op.create_index(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint([
            "tenant_id"
        ], [f"{SCHEMA}.tenants.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(op.f("ix_faqs_tenant_id"), "faqs", ["tenant_id"], schema=SCHEMA)
    ...
    op.create_index(op.f("ix_usage_tenant_id"), "usage", ["tenant_id"], schema=SCHEMA)
    op.create_index(
    ...
    if bind.dialect.name == "postgresql":
        qualified = f'"{SCHEMA}"."unavailability"'
        op.execute(
            sa.text(
                f"""
...
def downgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        ...
        op.execute(
            sa.text(
                ...
                f"DROP CONSTRAINT IF EXISTS {UNAVAILABILITY_EXCLUSION}"
            )
        )
    ...
    _drop_enum("appt_status_enum")
    _drop_enum("role_enum")
    _drop_extension(BTREE_GIST_EXTENSION)
    _drop_extension(VECTOR_EXTENSION)
