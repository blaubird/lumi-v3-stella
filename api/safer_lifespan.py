                ...
                if head_revisions:
                    ...
                    with engine.connect() as conn:
                        ...
                        if table_exists:
                            ...
                            conn.commit()
                            log.info(f"Successfully reset alembic_version to {current_head}")
                            ...
                            log.info(f"Stamped database with current head {current_head}")
                else:
                    log.error("No head revisions found")
                    raise
            except Exception as recovery_error:
                log.error(f"Failed to recover from migration error: {str(recovery_error)}")
                raise
        else:
