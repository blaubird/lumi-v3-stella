...
async def get_tenants(
    ...
    try:
        logger.info(
            "Starting tenant retrieval", extra={"page": page, "page_size": page_size}
        )
        ...
        logger.info(
            ...
            extra={"page": page, "page_size": page_size, "count": len(tenants)},
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while retrieving tenants: {str(e)}",
        )
...
async def create_tenant(tenant: TenantCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if existing:
            logger.warning(
                "Tenant creation failed: phone_id already exists",
                extra={"phone_id": tenant.phone_id},
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while creating tenant: {str(e)}",
        )
...
async def get_tenant(tenant_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while retrieving tenant: {str(e)}",
        )
    ...
    try:
        ...
        if tenant.phone_id is not None:
            ...
            if tenant.phone_id != db_tenant.phone_id:
                existing = (
                    db.query(Tenant).filter(Tenant.phone_id == tenant.phone_id).first()
                )
                if existing:
                    logger.warning(
                        "Tenant update failed: phone_id already exists",
                        extra={"phone_id": tenant.phone_id},
                    )
                    raise HTTPException(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while updating tenant: {str(e)}",
        )
...
async def get_tenant_messages(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while retrieving tenant messages: {str(e)}",
        )
...
async def get_tenant_faqs(tenant_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while retrieving tenant FAQs: {str(e)}",
        )
...
async def create_faq(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while creating FAQ: {str(e)}",
        )
...
async def get_tenant_usage(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error while retrieving tenant usage: {str(e)}",
        )
...
async def bulk_import_faq(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error during bulk FAQ import: {str(e)}",
        )
...
async def generate_embedding_for_faq(
    ...
    try:
        ...
        if not faq:
            logger.error(
                "FAQ not found for embedding generation",
